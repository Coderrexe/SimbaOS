...
export function CommandBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 md:left-16 right-0 h-auto md:h-14 glass-strong border-t border-[hsl(var(--border))] z-30 flex flex-col md:flex-row items-stretch md:items-center px-3 md:px-6 py-2 md:py-0 gap-2 md:gap-6">
      {/* Mode Switcher */}
      <div className="flex gap-1 p-1 rounded-[var(--radius-lg)] bg-[hsl(var(--surface2))] w-full md:w-auto">
        {modes.map((m) => {
          const Icon = m.icon;
          return (
            <button
              ...
              className={cn(
                "relative flex items-center justify-center md:justify-start gap-1 md:gap-2 px-2 md:px-3 py-1.5 rounded-[var(--radius)] text-xs font-medium transition-all flex-1 md:flex-initial",
                mode === m.id
                  ? "text-[hsl(var(--accent))]"
      ...
      {/* Focus Timer Status */}
      {focusTimer.active && (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-[var(--radius-lg)] bg-[hsl(var(--accent-muted))] text-[hsl(var(--accent))] w-full md:w-auto justify-center md:justify-start">
      ...
      {/* Quick Capture */}
      <form onSubmit={handleCapture} className="flex-1 w-full md:max-w-md">
      ...
      <div className="hidden lg:block text-xs text-subtle whitespace-nowrap">
